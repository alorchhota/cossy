...
class Rserve_Connection {
	...
	public function createFile($filepath) {
		...
		$data = _rserve_make_data(self::DT_STRING, $filepath);
	...
	public function writeFile($stream) {
		...
		/*
		$byte_array = unpack('C*', $stream);
		echo "==== count:" . count($byte_array) . "=====";
		$s = chr(self::DT_BYTESTREAM & 255); // [0]  Type
		$s .= mkint24(count($byte_array)); // [1] Length (24bits)
		
		foreach ($byte_array as &$value) {
			$s .= chr($value);
		}

		$r = $this->sendCommand(self::CMD_writeFile, $s);
		
		return !$r['is_error'];
		*/
		
		$data = _rserve_make_data(self::DT_BYTESTREAM, $stream);
